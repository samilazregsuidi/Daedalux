...
%token  SPEC EVENTUALLY ALWAYS GLOBALLY FINALLY UNTIL NEXT LTL BLTL K FMULTILTL /* TCTL */
...
prop	: LTL NAME '{' ltl_expr '}'						{	/*DBUG("REDUCE: one_decl -> unit\n")*/
															auto sym = new ltlSymNode($2, $4, nbrLines);
															(*globalSymTab)->insert(sym);
		...
		| BLTL NAME '{' bltl_expr '}'					{
															/*DBUG("REDUCE: one_decl -> unit\n")*/
															auto sym = new bltlSymNode($2, $4, nbrLines);
															(*globalSymTab)->insert(sym);
															stmnt* decl = new bltlDecl(sym, nbrLines);
															assert(decl);
															*program = stmnt::merge(*program, decl);
														}
		| FMULTILTL NAME variant_quants '{' ltl_expr '}' { 	/*DBUG("REDUCE: one_decl -> unit\n")*/
...
ltl_expr	: GLOBALLY expr	%prec NEG					{ $$ = new exprGlobally($2, nbrLines); 	}
			| FINALLY expr %prec NEG					{ $$ = new exprFinally($2, nbrLines); 	}
			| NEXT expr	%prec NEG						{ $$ = new exprNext($2, nbrLines); 		}
			| expr UNTIL expr							{ $$ = new exprUntil($1, $3, nbrLines); }
			| expr IMPLIES expr							{ $$ = new exprImplies($1, $3, nbrLines); }
			;

bltl_expr	: GLOBALLY k_steps expr %prec NEG			{ $$ = new exprBoundedGlobally($2, $3, nbrLines); 	}
			| FINALLY k_steps expr %prec NEG 			{ $$ = new exprBoundedFinally($2, $3, nbrLines); 	}
			| NEXT expr %prec NEG						{ $$ = new exprNext($2, nbrLines); 		}
			| expr UNTIL expr							{ $$ = new exprUntil($1, $3, nbrLines); }
			| expr IMPLIES expr							{ $$ = new exprImplies($1, $3, nbrLines); }
			;

k_steps		: '{' varref GT expr '}'				{ $$ = new exprGT ($2, $4, nbrLines); }
			| '{' varref LT expr '}'				{ $$ = new exprLT ($2, $4, nbrLines); }
			| '{' varref GE expr '}'				{ $$ = new exprGE ($2, $4, nbrLines); }
			| '{' varref LE expr '}'				{ $$ = new exprLE ($2, $4, nbrLines); }
			| '{' varref EQ expr '}'				{ $$ = new exprEQ ($2, $4, nbrLines); }
			| '{' varref NE expr '}'				{ $$ = new exprNE ($2, $4, nbrLines); }
			;
...
variant_expr	: '{' NAME '}' '[' expr ']'		{ auto sym = new variantSymNode(nbrLines, $2, $5); (*globalSymTab)->insert(sym); $$ = new exprVarRefName($2, sym, nbrLines); }
				//| '{' NAME '}' ASGN '[' feat_expr ']'		{ auto sym = new variantSymNode(nbrLines, $2, $6); (*globalSymTab)->insert(sym); $$ = new exprVarRefName($2, sym, nbrLines); }
				//| NAME									{ assert(false); /*find symbol in symbol table -> assert if null*/		         }
				//| NAME ASGN feat_expr						{ assert(false); /*find symbol in symbol table and assigh expr -> assert if null*/ }
