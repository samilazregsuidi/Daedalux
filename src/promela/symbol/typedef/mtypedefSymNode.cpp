...
void mtypedefSymNode::acceptVisitor(symTabVisitor * visitor) { visitor->visitMtypedef(this); }

void mtypedefSymNode::acceptVisitor(symTabConstVisitor * visitor) const { visitor->visitMtypedef(this); }
...
mtypedefSymNode::mtypedefSymNode(int lineNb) : symbol(symbol::T_MTYPE_DEF, "mtype", lineNb) {}

std::string mtypedefSymNode::getTypeName(void) const { return "mtypedef"; }

std::string mtypedefSymNode::getBasicTypeName(void) const { return "mtypedef"; }

int mtypedefSymNode::getTypeSize(void) const { return 0; }
...
int mtypedefSymNode::getLowerBound(void) const { return 0; }

const std::unordered_map<std::string, cmtypeSymNode *> & mtypedefSymNode::getMTypeList(void) const { return mtypes; }
...
int mtypedefSymNode::getIntValueOffset(void) const
{
  int offset = INT_MAX;
  for (auto cmtype : mtypes) {
    auto value = cmtype.second->getIntValue();
    if (value < offset)
      offset = value;
  }
  return offset;
