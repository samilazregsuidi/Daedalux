...
process::process(const seqSymNode * sym, const fsmNode * start, byte pid, unsigned int index)
    : thread(variable::V_PROC, sym, start, index), pid(pid)
{
}

process::process(const process & other) : thread(other), pid(other.pid) {}
...
void process::setProgState(progState * newS) { setParent(newS); }

progState * process::getProgState(void) const { return dynamic_cast<progState *>(getParent()); }

byte process::getPid(void) const { return payLoad ? variable::getTVariable<PIDVar *>("_pid")->getValue() : pid; }
...
bool process::isAccepting(void) const { return false; }

bool process::safetyPropertyViolation(void) const { return false; }

state * process::getNeverClaim(void) const { return parent ? dynamic_cast<state *>(parent)->getNeverClaim() : nullptr; }
  ...
  for (auto edge : node->getEdges()) {
    ...
    if (eval(edge, EVAL_EXECUTABILITY) > 0) {
      ...
      if (edge->getExpression()->getType() == astNode::E_STMNT_CHAN_SND) {
        ...
        // featuresOut contains all the outgoing features from now on, included the ones of the response that satisfy the base
        // FD (those may not satisfy the modified FD, though). *allProductsOut == 1 if the outgoing features reference all the
        // products.

        for (auto response : responses) {
          // conjunct *= dynamic_cast<progTransition*>(response)->getEdge()->getFeatures();
          // if((conjunct * s->stateMachine->getFD()).IsOne())
          // res.push_back(new RVTransition(s, const_cast<process*>(this), edge, conjunct,
          // dynamic_cast<progTransition*>(response)));
...
Apply:
  ...
  case (astNode::E_STMNT_CHAN_SND): {
    ...
    if (chan->isRendezVous()) {
      ...
      // Note that the features of the resulting state will be: "state->features & request_transition->features &
      // response_transition->features" Also, applying this transition will free _handshake_transit (because of calling
      // "channelReceive()"). Furthermore, the number of message in the rendezvous channel will be 0.
  ...
  case (astNode::E_STMNT_CHAN_RCV): {
    ...
    auto recvChanName = recvStmnt->getChan();
    auto recvChan = getChannel(recvChanName);

    if (recvChan->isRendezVous())
      // If the channel is a rendez-vous channel, we check if the rendez-vous can be completed.
      assert(s->hasHandShakeRequest() && s->getHandShakeRequestChan() == recvChan);
    else
      assert(!s->hasHandShakeRequest());

    recvChan->receive(getVariables(recvStmnt->getRArgList()));
    if (s->getHandShakeRequestChan() == recvChan)
      s->resetHandShake();
  ...
  // Todo so, we assert that the destination of the transition is either nullptr or this.
  // assert(trans->dst == nullptr || trans->dst == this);
  trans->dst = this;

  // std::cout << this->getFullName() << "::apply (" << oldLocation << ", " << dynamic_cast<const
  // processTransition*>(trans)->getEdge()->getLineNb() << ", " << getLocation() << ")" << std::endl;
}

int process::eval(const fsmEdge * edge, byte flag) const { return eval(edge->getExpression(), flag); }
...
void process::print(void) const
{
  auto node = getFsmNodePointer();
  if (node)
    printf("0x%-4lx:   %s pid  %-13u @ NL%02u\n", getOffset(), getFullName().c_str(), getPid(), node->getLineNb());
  else
    printf("0x%-4lx:   %s pid  %-13u @ end\n", getOffset(), getFullName().c_str(), getPid());

  variable::print();
}

void process::printCSVHeader(std::ostream & out) const { out << getFullName() << ","; }

void process::printCSV(std::ostream & out) const { out << getLocation() << ","; }

void process::accept(stateVisitor * visitor) { visitor->visit(this); }