...
channel::channel(const channel * other) : primitiveVariable(other) { assert(getSizeOf() == other->getSizeOf()); }

variable * channel::deepCopy(void) const { return new channel(this); }

channel::~channel() {}

size_t channel::getSizeOf(void) const { return variable::getSizeOf(); }
...
bool channel::isRendezVous(void) const { return getCapacity() == 0; }

bool channel::isFull(void) const { return len() == getCapacity(); }

bool channel::isEmpty(void) const { return len() == 0; }
...
byte channel::getCapacity(void) const { return dynamic_cast<const chanSymNode *>(varSym)->getCapacity(); }
...
CIDVar::CIDVar(const cidSymNode * sym, unsigned int bound) : primitiveVariable(sym, bound), ref(nullptr) {}
...
channel * CIDVar::getRefChannel(void) const { return ref; }
...
CIDVar::operator std::string(void) const { assert(false); }

void CIDVar::print(void) const { assert(false); }