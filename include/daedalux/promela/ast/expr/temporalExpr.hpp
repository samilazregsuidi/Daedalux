...
//E_EXPR_BOUNDED_GLOBALLY,	// child[0] = E_EXPR_*
class exprBoundedGlobally : public exprUnaryTemporal
{
public:
	exprBoundedGlobally(expr* bound, expr *mExpr, int lineNb)
		: exprUnaryTemporal(astNode::E_EXPR_BGLOBALLY, mExpr, lineNb)
	{
		addChild("bound", bound);
	}

	expr* getBound(void) const {
		return dynamic_cast<expr*>(getChild("bound"));
	}

	operator std::string() const override {
		assert(getExpr());
		return "[]{"+ std::string(*getBound())+"}" + std::string(*getExpr());
	}

	std::string getTypeDescr(void) const override {
		return "Bounded globally temporal operator (E_EXPR_BOUNDED_GLOBALLY)";
	}

	expr* deepCopy(void) const override {
		exprBoundedGlobally* copy = new exprBoundedGlobally(*this);
		copy->copyChildren(*this);
		return copy;
	}
};

//E_EXPR_BOUNDED_FINALLY,	// child[0] = E_EXPR_*
class exprBoundedFinally : public exprUnaryTemporal
{
public:
	exprBoundedFinally(expr* bound, expr *mExpr, int lineNb)
		: exprUnaryTemporal(astNode::E_EXPR_BFINALLY, mExpr, lineNb)
	{
		addChild("bound", bound);
	}
	...
	std::string getTypeDescr(void) const override {
		return "Bounded finally temporal operator (E_BOUNDED_EXPR_FINALLY)";
	}

	expr* deepCopy(void) const override {
		exprBoundedFinally* copy = new exprBoundedFinally(*this);
		copy->copyChildren(*this);
		return copy;
	}
};
...
public:
	...
	operator std::string() const override {
		assert(getExpr());
		variantSymNode* sym = dynamic_cast<variantSymNode*>(dynamic_cast<exprVarRefName*>(getExpr())->getSymbol());
		return "A{" + std::string(*getExpr())+"}["+ std::string(*sym->getInitExpr()) +"]";
	}
...
public:
	...
	operator std::string() const override {
		assert(getExpr());
		variantSymNode* sym = dynamic_cast<variantSymNode*>(dynamic_cast<exprVarRefName*>(getExpr())->getSymbol());
		return "E{" + std::string(*getExpr())+"}["+ std::string(*sym->getInitExpr()) +"]";
	}
